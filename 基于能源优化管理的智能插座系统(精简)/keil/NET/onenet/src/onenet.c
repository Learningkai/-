...
//    // 声明一个字符数组 text，大小为48字节
//    char text[48];
//    
//    // 清空 text 数组，将所有字节设置为 0
//    memset(text, 0, sizeof(text));
//    
//    // 将初始的 JSON 片段拷贝到 buf 中
//    strcpy(buf, "{\"id\":\"123\",\"params\":{");
//    
//    // 清空 text 数组
//    memset(text, 0, sizeof(text));
//    
//    // 使用 sprintf 函数将格式化的字符串写入 text 中
//    // 这里的字符串格式为 "cur\":{\"value\":3}"
//    sprintf(text, "\"cur\":{\"value\":%d}", 3);
//    
//    // 将 text 拼接到 buf 的末尾
//    strcat(buf, text);
//    
//    // 如果需要添加更多的参数，可以取消注释以下代码并进行修改
//    // 清空 text 数组
//    // memset(text, 0, sizeof(text));
//    // 使用 sprintf 函数将另一个格式化的字符串写入 text 中
//    // 这里的字符串格式为 "kai\":{\"value\":true}"
//    // sprintf(text, "\"kai\":{\"value\":%s},", "true");
//    // 将 text 拼接到 buf 的末尾
//    // strcat(buf, text);
//    
//    // 在 buf 的末尾添加 JSON 结束标记
//    strcat(buf, "}}");
//    
//    // 返回 buf 的长度
//    return strlen(buf);
    ...
    char text[48];  // 增加 text 的大小以适应更多内容
		...
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
              ...
              //Led_Set(LED_ON);
				 ...
				 if (ele_json != NULL)
					{
							HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);//改变电平
